...
public class ShowSeatFeignClientFallbackFactory implements FallbackFactory<ShowSeatFeignClient> {
    ...
    public ShowSeatFeignClient create(Throwable cause) {
        ...
        return new ShowSeatFeignClient() {
            ...
            @Override
            public ApiResponseDto<ShowResponseDto> getShowById(String showId) {
                log.error("Fallback: Unable to fetch show {} from ShowSeat service. Error: {}",
                        showId, cause.getMessage());

                // Throw exception as show details are critical for ticket display
                throw new RuntimeException(
                        "Show service temporarily unavailable. Cannot fetch ticket details.");
            }
        };
    }
}
