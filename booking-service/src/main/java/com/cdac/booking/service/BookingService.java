...
public class BookingService {
	...
	private CreateBookingResponse createNewBooking(String idempotencyKey, CreateBookingRequest request) {
		...
		BigDecimal totalAmount = BigDecimal.valueOf(confirmedSeats.size()).multiply(BigDecimal.valueOf(150));
		...
		confirmedSeats.forEach(seat -> bookingSeatRepository.save(
				BookingSeat.builder().bookingId(booking.getId()).seatNo(seat).price(BigDecimal.valueOf(150)).build()));
