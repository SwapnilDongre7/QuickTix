...
public class TheatreController {
    ...
    // Get theatre by ID (used for inter-service communication)
    @GetMapping("/{theatreId}")
    public Theatre getById(@PathVariable("theatreId") Long theatreId) {
    ...
    public ResponseEntity<Void> delete(@PathVariable("theatreId") Long theatreId,
            @org.springframework.web.bind.annotation.RequestHeader("Authorization") String token) {
        if (token != null && token.startsWith("Bearer ")) {
            token = token.substring(7);
        }
        ...
        @SuppressWarnings("unchecked")
        List<String> roles = claims.get("roles", List.class);
        theatreService.deleteTheatre(theatreId, userId, roles);
