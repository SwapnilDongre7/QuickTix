...
public class TheatreOwnerController {
    ...
    /**
     * Apply to become a theatre owner.
     * Allows any authenticated user to register themselves.
     * Creates a PENDING application.
     * 
     * Security: Uses X-User-Id header from Gateway (extracted from JWT)
     * to ensure users can only register themselves.
     */
    @PostMapping("/register")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<TheatreOwner> register(@RequestHeader("X-User-Id") Long userId) {
        TheatreOwner owner = theatreOwnerService.registerOwner(userId);
        return ResponseEntity.ok(owner);
    }
    ...
    @GetMapping("/pending")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<com.movie.theatre.dto.PendingApplicationDTO>> getPendingApplications() {
        return ResponseEntity.ok(theatreOwnerService.getPendingApplications());
    }
