...
public class IdentityFeignClientFallbackFactory implements FallbackFactory<IdentityFeignClient> {
    ...
    public IdentityFeignClient create(Throwable cause) {
        ...
        return new IdentityFeignClient() {
            ...
            @Override
            public com.movie.theatre.dto.UserInfoResponse getUserById(Long userId) {
                log.error("Fallback: Unable to get info for user {}. Error: {}",
                        userId, cause.getMessage());
                // Return placeholder so listing doesn't fail completely
                return new com.movie.theatre.dto.UserInfoResponse(userId, "Unavailable", "Unavailable");
            }
        };
    }
}
