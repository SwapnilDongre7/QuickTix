...
public class TheatreService {
    ...
    public void deleteTheatre(Long theatreId, Long userId, List<String> roles) {
        Theatre theatre = theatreRepository.findById(theatreId)
                .orElseThrow(() -> new ResourceNotFoundException("Theatre not found"));
        ...
        if (!isAdmin) {
            if (!isOwner) {
                throw new UnauthorizedOperationException("User is not authorized to delete theatre");
            }
            // Check if this user owns the theatre
            if (!theatre.getOwner().getUserId().equals(userId)) {
                throw new UnauthorizedOperationException("You can only delete your own theatres");
            }
        }
